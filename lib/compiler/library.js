	...
	if (symboltable.classmap) {
		...
		Object.keys(symboltable.classmap).forEach(function(name){
			...
			if (!clscode.length && isClassLike) {
				...
				clscode.push('{');
				clscode.push('\tauto abstractObject = ToNative(object);');
				clscode.push('\t_classMap.erase(_classMap.find(abstractObject->getObject()));');
				clscode.push('\tabstractObject->release();');
				clscode.push('}');
				clscode.push('');
				...
				clscode.push('{');
				clscode.push('\tif(_classMap[instance]) {');
				clscode.push('\t	return _classMap[instance];');
				clscode.push('\t}');
				...
				clscode.push('\t_classMap[instance] = JSObjectMake(ctx, RegisterClass(), po);');
				clscode.push('\treturn _classMap[instance];');
				clscode.push('}');
				clscode.push('');
			...
			code.push('');
			code.push('#include <hyperloop.h>');
			code.push('#include <unordered_map>');

			library.prepareIncludes && library.prepareIncludes(options,metabase,state,code);
			code.push('\n');
			code.push('static std::unordered_map<void*, JSValueRef> _classMap;')
