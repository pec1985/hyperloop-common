...
function generateLibrary (options, build_opts, arch, state, library, uncompiledFiles, compiledFiles, callback) {
	...
	Object.keys(compiledFiles).forEach(function(key){
		...
		// give platform library a chance to append something
...
function generateMain (options, state, obj, symbols, symbolnames, externs, cleanup) {
	...
	Object.keys(state.symbols).forEach(function(name){
		...
		// only push code if this symbol is in the current file, but
		// if for some reason this symbol has no "filename" push it anyway
		if(!symbol.filename || symbol.filename == obj.jsfilename) {
			...
			symbols.push('JSStringRelease('+name+'Property);');
			symbols.push('');
		}
