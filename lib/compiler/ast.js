	...
	var transformer = new Uglify.TreeTransformer(null, function(node, descend){
		...
		if (node instanceof Uglify.AST_New) {
			...
			if (!isBuiltinObject(name) && library.isValidSymbol(state, name)) {
				return generateNewConstructor(library, state, name, node, jsfilename);
			}
			...
			if (varStatic) {
				return generateStaticGetter(library, state, varStatic.name, varStatic.property, node, jsfilename);
			}
			...
			if (cls && !jsgen.isBuiltinFunction(field) && property) {
				return generateGetterCall(library, state, cls, property, instance, node, jsfilename);
			}
		}
			...
			if (node.left.hyperloop && node.left.hyperloop.type==='getter') {
				...
				if (right.hyperloop && right.hyperloop.type==='setter') {
						...
						right = generateGetterCall(library, state, right.hyperloop.class, property, right.args[0], right, jsfilename);
						if (cls && !jsgen.isBuiltinFunction(field) && library.findProperty(state.metabase, cls, field)) {
							var result = generateSetterCall(library, state, cls, field, node.left.hyperloop.instance, right, node, jsfilename);
							return new Uglify.AST_Array({elements:[oldAssign, result]});
						}
				}
				if (cls && !jsgen.isBuiltinFunction(field) && library.findProperty(state.metabase, cls, field)) {
					return generateSetterCall(library, state, cls, field, node.left.hyperloop.instance, right, node, jsfilename);
				}
			}
			...
			if (node.start.value === 'Hyperloop') {
				...
				if (command) {
					return compileCommand(options,state,library,arch,node,command,dict,jsfilename);
				}
			}
			// handle cast
			...
			if (nodeRef) {
				...
				if (nodeRef.variable === 'super') {
					return generateSuperMethodCall(library, state, nodeRef['superclass'], nodeRef['class'], node.expression.property, node, jsfilename);
				}
				...
				if (node.expression && name && !isBuiltinObject(name) && library.isValidSymbol(state, name)) {
					return generateFunctionCall(library, state, name, node, jsfilename);
				}
					...
					if (node.expression.expression.hyperloop && node.expression.expression.hyperloop.method) {
						return generateMethodCall3(options, library, state, node.expression.expression.hyperloop, node, jsfilename);
					}
					...
					if (undeclared && name && !isBuiltinObject(name) && library.isValidSymbol(state, name)) {
						return generateClassMethodCall(library, state, name, node.expression.property, node, jsfilename);
					}
						...
						if (lookup) {
							...
							if (name && !isBuiltinObject(name) && library.isValidSymbol(state, name)) {
								return generateMethodCall2(library, state, name, node, jsfilename);
							}
					...
					if (node.expression) {
						...
						if (undeclared && !isBuiltinObject(cls) && library.isValidSymbol(state,cls)) {
							return generateClassMethodCall(library, state, cls, methodName, node, jsfilename);
						}
					}
				}
				...
				if (node.expression && node.expression.property && node.expression.expression) {
					...
					if (val && key.value !== 'Hyperloop') {
						...
						return generateMethodCall2(library, state, cls, node, jsfilename);
					}
...
function generateNewConstructor(library, state, name, node, jsfilename) {
	var fn,
		key,
		symbol = library.getConstructorSymbol(state,name,node,fail);
	fn = symbol.symbolname;
	key = state.obfuscate ? jsgen.obfuscate(fn) : fn;
	if(jsfilename) symbol.filename = jsfilename;
	state.symbols[key] = symbol;
...
function generateSuperMethodCall(library, state, superclass, classname, methodname, node, jsfilename) {
...
function generateMethodCall(library, state, name, node, jsfilename) {
	var varname = node.start.value,
...
function generateMethodCall2(library, state, name, node, jsfilename) {
	var varname = node.start.value,
		expr = node.expression.property,
		fn = jsgen.generateMethodName(name, expr),
...
function generateMethodCall3(options, library, state, methodinfo, node, jsfilename){
	var varname = node.start.value,
		expr = node.expression.property,
...
function generateClassMethodCall(library, state, cls, name, node, jsfilename) {
	var varname = node.start.value,
...
function generateFunctionCall(library, state, name, node, jsfilename) {
...
function generateStaticGetter(library, state, classname, property, node, jsfilename) {
...
function generateGetterCall(library, state, classname, property, instance, node, jsfilename) {
	var fn = jsgen.generateGetterName(classname, property.name),
		key = state.obfuscate ? jsgen.obfuscate(fn) : fn,
		symbol = library.getGetterSymbol(state,classname,property.name,null,fn,node,fail);
...
function generateSetterCall(library, state, classname, property, instance, value, node, jsfilename) {
 ...
 */
function compileCommand(options, state,library,arch,node,command,dict,jsfilename) {
	switch (command) {
		case 'defineClass': {
			if (dict.build) {
		...
		case 'method': {
			if (dict.call) {
				...
				var result = library.defineMethod(options,state,arch,node,dict,fail,jsfilename);
